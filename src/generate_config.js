...
function rootFolderList(basePath) {
  ...
  const noempty = subfolders.reduce((total, folder) => {
    return getFilesUnderFolder(path.join(basePath, folder)).length ? total.concat(folder) : total
  }, [])

  return noempty
}

function rootFileList(basePath) {
  const fileList = fs
    .readdirSync(basePath)
 ...
 */
function _CreateVitePressConfigFile() {
