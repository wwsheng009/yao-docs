...
除了在应用目录`mcps`下创建全局的MCP服务器，Assistants can define their own namespaced MCP servers in the `mcps/` directory:
...
不同于stdio/sse/http协议，在yao应用中，可以直接使用yao处理器作为MCP工具。

需要把transport设置为process。
