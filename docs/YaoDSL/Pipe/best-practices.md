...
[
  {
    ...
    "process": {
      ...
      "args": ["{{ $in[0] }}", "email_pattern", "required_fields"]
    },
    "output": {
      "is_valid": "{{ $out.valid }}",
      "errors": "{{ $out.errors }}",
...
```json
{
  ...
  "label": "订单处理流程",
  "description": "处理用户订单的完整流程，包括：订单验证、库存检查、支付处理、订单确认",
  "nodes": [
    {
      "name": "validate-order",
      "label": "订单验证",
...
```json
{
  ...
  "version": "1.2.0",
  "label": "支付处理器 v1.2.0",
  "changelog": "v1.2.0: 添加新的支付方式; v1.1.0: 优化错误处理; v1.0.0: 初始版本"
}
```
...
[
  ...
  },
  {
    ...
    "output": {
      ...
      "performance_ok": "{{ $out.duration < 5000 }}"
    }
  }
]
```
...
// ❌ 危险：可能导致无限循环
[
  {"name": "node1", "goto": "node2"},
  {"name": "node2", "goto": "node1"}
]
...
// ✅ 安全：使用条件跳出循环
{
  ...
  "goto": "{{ $out.has_more ? 'process-item' : 'EOF' }}"
}
```
...
// ✅ 好的做法：使用全局配置
{
  ...
  "nodes": [
    {
      "name": "get-threshold",
      "input": ["{{ $global.processing_threshold ?? 100 }}"]
    },
    {
      "name": "apply-logic",
      "switch": {
