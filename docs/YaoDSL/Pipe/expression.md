...
> ⚠️ **重要**: 配置示例使用 **JSON 格式**，不是 YAML！
>
> 📖 查看 [JSON 格式说明](./JSON-格式说明.md) 了解详细的转换指南

Pipe 使用强大的表达式引擎来处理数据绑定、条件判断和动态计算。表达式引擎基于 Expr 库实现，支持丰富的语法和功能。
...
| 变量      | 类型   | 说明          | 示例                    |
| --------- | ------ | ------------- | ----------------------- |
| `$global` | map    | 全局数据      | `{{ $global.user_id }}` |
| `$input`  | array  | Pipe 输入数据 | `{{ $input[0] }}`       |
| `$output` | any    | Pipe 输出数据 | `{{ $output.result }}`  |
| `$sid`    | string | 会话ID        | `{{ $sid }}`            |
...
| 变量               | 说明         | 示例                             |
| ------------------ | ------------ | -------------------------------- |
| `$node.<name>.in`  | 指定节点输入 | `{{ $node.validate.in[0] }}`     |
| `$node.<name>.out` | 指定节点输出 | `{{ $node.process.out.result }}` |
