...
export const DOCS_URL = 'https://github.com/wwsheng009/yao-docs'
...
async function fetchContributors(page = 1) {
  const additional = ['wwsheng009']

  const collaborators: string[] = []
  const data = await $fetch<{ login: string }[]>(`https://api.github.com/repos/wwsheng009/yao-docs/contributors?per_page=100&page=${page}`, {
    method: 'get',
    headers: {
